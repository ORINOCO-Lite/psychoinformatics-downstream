---
url: {'annotation_tag': 'xyzrins:annotation-tags/psyinf-ns-id', 'annotation_value': 'instruments/shacl-vue'}
title: shacl-vue
persons:
- stephan-heunis
params:
  graphRootNodePID: xyzrins:instruments/b8dc8b0a-4408-46f4-9f47-c1d3ae747fac
  pid: xyzrins:instruments/b8dc8b0a-4408-46f4-9f47-c1d3ae747fac
  doi: null
  date: null
  source_code_url: https://hub.psychoinformatics.de/orinoco/shacl-vue
  documentation_url: https://shacl-vue.psychoinformatics.de/
  title: shacl-vue
  description: A VueJS application for automatic generation of user interfaces from
    SHACL
  kind: Software
  author:
  - pid: xyzrins:persons/stephan-heunis
    given_name: Stephan
    family_name: Heunis
  topic: []
  license:
  - pid: spdxlic:MIT
    label: MIT License
    url: https://spdx.org/licenses/MIT

---

